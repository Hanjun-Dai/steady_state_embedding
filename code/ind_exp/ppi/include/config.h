...
struct cfg
{
    static bool is_regression, avg, sparse_feat, norm_embed;
    static int iter;
    ...
    static const char *save_dir, *data_root, *f_feat, *f_idx, *f_score, *f_label, *saved_model;
    static char *f_train_idx, *f_test_idx;

    static void LoadParams(const int argc, const char** argv)
    {
        const char* input_train = nullptr, *input_test = nullptr;
        for (int i = 1; i < argc; i += 2)
        {
            if (strcmp(argv[i], "-train_idx") == 0)
		        input_train = argv[i + 1];
            ...
            if (strcmp(argv[i], "-feat") == 0)
		        f_feat = argv[i + 1];
            if (strcmp(argv[i], "-saved_model") == 0)
            ...
            if (strcmp(argv[i], "-norm_embed") == 0)
                norm_embed = atoi(argv[i + 1]);                         
        ...
        assert(input_train && input_test);
        sprintf(f_train_idx, "%s", input_train);
        sprintf(f_test_idx, "%s", input_test);
        ...
        std::cerr << "norm_embed = " << norm_embed << std::endl;
