...
DataProvider::DataProvider(std::string _name, std::vector<int>& idxes, unsigned _batch_size)
{
    name = _name;
    Init(idxes, _batch_size);
}

DataProvider::DataProvider(std::string _name, int num_nodes, unsigned _batch_size)
{
    name = _name;
...
std::map< std::string, void* > DataProvider::GetNextBatch()
{
    ...
    inputs["node_input"] = &dense_m_node_feat;
