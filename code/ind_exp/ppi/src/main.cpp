...
void UpdateEmbedding(int node_idx, DTensor<mode, Dtype>& new_embed)
{
    ...
    old_embed.Axpby(0.1, new_embed, 0.89);
...
{    
    ...
    {
        int idx = cur_test_idxes[j];
        auto* cur_pred = mat_pred.data->ptr + j * cfg::num_labels;

        for (int k = 0; k < cfg::num_labels; ++k)
            binary_pred.data->ptr[j * cfg::num_labels + k] = (int)(cur_pred[k] > 0.5);
    }
...
void InitParams()
{
    ...
    param->value.Fill(0);  
    GetVar("init_w", cfg::dim_feat + 1, cfg::n_embed);
    GetVar("W", cfg::n_embed * 4 + 1, cfg::n_embed);
